...
export default function (
  ...
  const rollupConfig: RollupOptions = {
    ...
    plugins: [
      ...
      { name: "concat-jsx-files-to-file", async transform(code, id) {
          if(!id.includes(filePath)) return;
  ...
  return {
    ...
    config(config: UserConfig) {
      const i = (((config.build ||= {}).rollupOptions ||= {}).input ||= {})
      if(typeof i === "string") config.build.rollupOptions.input = {[i]: __dirname + "/" + i};
    ...
    async buildStart() {
      if (commandUsed === "build") await compile();
      else compile().then(); // don't wait for it
    },
    ...
    async handleHotUpdate({file, server}) {
      ...
      if (file.includes(filePath) || file.includes(outputFilePath)) return []
    }
  }
}
