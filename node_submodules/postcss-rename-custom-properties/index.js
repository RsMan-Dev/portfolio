...
function generateNextPropName(currIndex) {
    let res = "";
...
function newPropOrCurrent(prop, duplicates) {
    if (!duplicates.has(prop))
        duplicates.set(prop, { newName: generateNextPropName(duplicates.size), count: 0 });
    duplicates.get(prop).count++;
    return duplicates.get(prop).newName;
}
function duplicatesToPotentialEconomyResultStr(duplicates) {
    if (duplicates.size === 0)
        return 'No custom properties renamed.';
    let toR = `Potential economy of ${[...duplicates.entries()].reduce((acc, [old, { newName, count }]) => acc + (old.length - newName.length) * count, 0) / 1024}ko, renamed ${duplicates.size} custom properties: `;
    for (const [old, { newName, count }] of duplicates.entries())
        toR += `${old} -> ${newName} (${count}), `;
...
const plugin = (opts = {}) => {
    ...
    return {
        ...
        OnceExit(css) {
            ...
            if (opts.safeList)
                opts.safeList.forEach(prop => duplicates.set(prop, { newName: prop, count: 0 }));
            ...
            css.walkDecls(decl => {
                ...
                if (decl.prop.match(CUSTOM_PROPERTY_REGEX))
                    decl.prop = newPropOrCurrent(decl.prop, duplicates);
            });
            // report potential economy in octets
            if (process.env.NODE_ENV !== 'production')
                console.log(duplicatesToPotentialEconomyResultStr(duplicates));
